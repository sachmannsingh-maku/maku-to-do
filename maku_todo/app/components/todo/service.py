...
class ToDoService:
    async def create_todo_service(self,db:AsyncSession, current_user, todoData):
        return await create_todo_in_db(db, title = todoData.title, description = todoData.description, owner_id = current_user.id)

    async def list_todos_service(self,db:AsyncSession, current_user):
        return await get_todos_by_user(db, owner_id = current_user.id)

    async def get_todo_or_404(self, db:AsyncSession, todo_id:int):
    ...
    def verify_ownership(self,todo:Todo, current_user):
        if(todo.owner_id != current_user.id):
            raise HTTPException(status_code = status.HTTP_403_FORBIDDEN, detail= "Not authorized to access this todo")
        
    async def update_todo_service(self,db:AsyncSession, todo_id:int, todo_data, current_user):
        todo = await self.get_todo_or_404(db, todo_id)
        self.verify_ownership(todo, current_user)
    ...
    async def delete_todo_service(self, db: AsyncSession, todo_id: int, current_user,):
        todo = await self.get_todo_or_404(db, todo_id)
        self.verify_ownership(todo, current_user)
